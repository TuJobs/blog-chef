...
import { getReactionsByPostId, toggleReaction, createUser, getUserById } from "@/lib/sqlite";

// POST - Toggle reaction (like/unlike)
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { postId, userId, authorId, authorName, authorAvatar, type = "LIKE" } = body;
    ...
    if (!user) {
      user = await createUser({
        ...
        avatar: authorAvatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${userIdToUse}`,
      });
    }
