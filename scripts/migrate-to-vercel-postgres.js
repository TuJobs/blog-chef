  ...
  try {
    ...
    if (fs.existsSync(path.join(dataDir, "posts.json"))) {
      const posts = JSON.parse(
        fs.readFileSync(path.join(dataDir, "posts.json"), "utf8")
      );
      for (const post of posts) {
        await sql`
          ...
          VALUES (${post.id}, ${post.title}, ${post.content}, ${
          post.category
        }, ${post.tags || []}, ${post.images || []}, ${post.likes || 0}, ${
          post.views || 0
        }, ${post.comments || 0}, ${post.authorId}, ${
          post.status || "published"
        }, ${post.excerpt || ""}, ${post.createdAt}, ${post.updatedAt})
          ON CONFLICT (id) DO NOTHING
        `;
      }
    ...
    if (fs.existsSync(path.join(dataDir, "comments.json"))) {
      const comments = JSON.parse(
        fs.readFileSync(path.join(dataDir, "comments.json"), "utf8")
      );
      for (const comment of comments) {
        await sql`
          INSERT INTO comments (id, content, likes, post_id, author_id, created_at, updated_at) 
          VALUES (${comment.id}, ${comment.content}, ${comment.likes || 0}, ${
          comment.postId
    ...
    if (fs.existsSync(path.join(dataDir, "reactions.json"))) {
      const reactions = JSON.parse(
        fs.readFileSync(path.join(dataDir, "reactions.json"), "utf8")
      );
      for (const reaction of reactions) {
        await sql`
          INSERT INTO reactions (id, type, post_id, author_id, created_at) 
          VALUES (${reaction.id}, ${reaction.type || "LIKE"}, ${
          reaction.postId
        }, ${reaction.authorId}, ${reaction.createdAt})
